...
// TODO
// rand, gaussian, lognormal, distortion (as with Zeitgeber), horizontal and vertical blur, brightness enhancement

void main( void ) {
    vec2 uv0 = vec2( gl_FragCoord.s / res.s, 1. - ( gl_FragCoord.t / res.t ) ); // invert y-axis for Processing

    vec2 uv = 2. * uv0 - 1.; // [-1,1]

    vec2 off = 1. / res; // 1-texel offset for convolution filtering

    float c = 0.;

    uv.t +=   /*mod( */sin(/* i  * */ 1./res.s * .1 * PI * t  + pow( uv.s, 10. /*1. * sin( mod( t, 100. ) ) */ ) + uv.x); //, uv.y );
    uv.s +=   /*mod( */sin(/* i  * */ 1./res.t * -.5 * PI * t  + pow( uv.t, 10. /*1. * sin( mod( t, 100. ) ) */ ) + uv.y);
